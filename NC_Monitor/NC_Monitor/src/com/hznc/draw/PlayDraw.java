...
public class PlayDraw {
	...
	public void axis(int x0, int y0, int nx, int sx, int ny, int sy, int c) {
		...
		y = ny * sy;
		for(i=0;i<=nx;i++){
			drawText(x0+i*sx-5, y0+35, c, 25, i+"");
			dotLine(x0 + i*sx, y0+13, x0 + i*sx, y0+7, c, 1);
		}
		for (i = 1; i < 2; i++){
			dotLine(x0, y0 - i * sy+10, x0 + x, y0 - i * sy+10, c, 1);
		}
		drawText(x0-30,y0-y*1/3+25,c,25,1+"");
		drawText(x0-30,y0-y*1/3,c,25,-1+"");
		drawText(x0-30,y0-y*2/3,c,25,0+"");
		drawText(x0-30,y0-y+25,c,25,1+"");
		drawText(20,y0+20,c,25,"kHz");
		dotLine(x0,y0-y*2/3,x0+x,y0-y*2/3,c,1);
		drawRect(x0, y0 - y, x, y*2/3, c, 1);
		drawRect(x0,y0-y*1/3+10,x,y*1/3,c,1);
	}
		...
		if (type > 1) {
			...
			for (i = 0; i <= n; i++){
				line(x1 + i * (x2 - x1) / n, y2 - 3, x1 + i * (x2 - x1) / n,
						y2 + 3, c1, 2);
				drawText(x1 + i * (x2 - x1) / n-13, y2 +30, c1, 25, ""+i);
			}
			drawText(x2+50,y2+10,c1,30,"kHz");
		}
	}
